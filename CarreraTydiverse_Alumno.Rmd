...
```{r}
cran <- cran %>% select(r_arch:country)
```
...
cran <- cran %>% select(-any_of(c("date", "size")))
...
```{r}
cran_plotly <- cran %>% filter(package == "plotly", version == "3.5.1")
```
...
```{r}
cran %>% mutate(size_mb = size / 2^20) %>%filter(size_mb <= 0.5) %>%arrange(desc(size_mb)) %>%print()
...
cran %>%
  ...
  labs(x = "País", y = "Tráfico generado (bytes)") +
  theme_minimal()
...
students <- students %>%pivot_longer(cols = c(male, female), names_to = "sex", values_to = "count")
...
```{r}
students2 <- students2 %>% pivot_longer(cols = starts_with("male") | starts_with("female"), names_to = c("sex", "class"), names_sep = "_", values_to = "count")
...
students3 <- students3 %>%pivot_longer(cols = starts_with("class"), names_to = "class", values_to = "score") %>%separate(class, into = c("class", "exam"), sep = "_") %>%pivot_wider(names_from = exam, values_from = score)
...
student_info <- students4 %>%select(id, name, class) %>%distinct()
...
ggplot(sat_tidy, aes(x = score_range, y = prop, fill = sex)) +
  ...
  labs(
    ...
    fill = "Sexo",
    title = "Proporción de estudiantes por rango de puntuación, sexo y parte del examen"
  ) +
  theme_minimal()
...
class(my_date)
new_fecha <- ymd("1989 May 17")
new_fecha
fecha <- mdy("March 12, 1975")
fecha
```
